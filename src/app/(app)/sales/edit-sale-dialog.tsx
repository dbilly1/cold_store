        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {dialog.items.map((item, idx) => {
              ...
              const hasBoxes = item.unit_type !== "boxes" && upb > 0;
              const price = parseFloat(item.unit_price) || 0;
              const disc = parseFloat(item.discount_amount) || 0;
              const qty = parseFloat(item.quantity) || 0;
              const qBoxes = parseFloat(item.quantity_boxes) || 0;
              const boxKgEquiv = hasBoxes ? qBoxes * upb : 0;
              const effectiveQty = item.unit_type === "boxes" ? qty : qty + boxKgEquiv;
              const lt = Math.max(0, effectiveQty * price - disc);
              return (
                    ...
                    <>
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => onItemChange(idx, { quantity_boxes: e.target.value })}
                          />
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-2">
                      ...
                      {qBoxes > 0 && (
                        <p className="text-xs text-slate-400">
                          {qBoxes} box{qBoxes !== 1 ? "es" : ""} × {upb} {item.unit_type}/box = {boxKgEquiv.toFixed(3)} {item.unit_type} added
                        </p>
                      )}
                    </>
                  ) : (
                    <div className="grid grid-cols-3 gap-2">
                      <div>
                        <Label className="text-xs">
                          {item.unit_type === "boxes" ? "Boxes" : `Qty (${item.unit_type})`}
                        </Label>
                        <Input
                          type="number"
                          min="0"
